    ...
    switch(g_packet.state.subState) {
        case ALL_OFF: // hold until button is pressed for long enough
            ...
            if(millis() - countdownStartTime > IGNITION_SAFE_TIME) {
                if(1 == g_controlBox.getMessage().allButtons) {
                    ...
                    digitalWrite(g_outPins[IGNITION_RELAY], RELAY_ON);
        ...
        case IGNIT_ON:
            if(millis() - countdownStartTime > VALVE_ON_TIME) {
                ...
                digitalWrite(g_outPins[OXIDIZER1_RELAY], RELAY_ON);
                digitalWrite(g_outPins[OXIDIZER2_RELAY], RELAY_ON);
        ...
        case VALVE_ON:
            if(millis() - countdownStartTime > IGNITION_OFF_TIME) {
                ...
                digitalWrite(g_outPins[IGNITION_RELAY], RELAY_OFF);
        ...
        case IGNIT_OFF:
            if(millis() - countdownStartTime > VALVE_OFF_TIME) {
                ...
                digitalWrite(g_outPins[OXIDIZER1_RELAY], RELAY_OFF);
                digitalWrite(g_outPins[OXIDIZER2_RELAY], RELAY_OFF);
        ...
        case VALVE_OFF:
            if(millis() - countdownStartTime > OXIDIZER_EMPTY_TIME) {
                ...
                digitalWrite(g_outPins[NITROGEN_RELAY], RELAY_ON);
            }
            break;

        case PURGING:
            if(millis() - countdownStartTime > CAMERA_TRIGGER_TIME) {
                digitalWrite(g_outPins[HIGH_SPEED_SIGNAL], HIGH);
            }

            if(millis() - countdownStartTime > OXIDIZER_EMPTY_TIME) {
                g_packet.state.subState = FINISHED;
                digitalWrite(g_outPins[NITROGEN_RELAY], RELAY_OFF);
            }
            break;
