...
def updateGraph(tag, newX, newY, xAxisTag=None, windowSeconds=5.0):
    ...
    if not hasattr(updateGraph, "dataBuffers"):
        ...
        updateGraph.dataBuffers = {}

    # Check if I have a place to store data for a specific tag
    if tag not in updateGraph.dataBuffers:
        # Creating a place to store data for a given tag
        updateGraph.dataBuffers[tag] = {"x": [], "y": []}

    # Appending data to the buffer
    buf = updateGraph.dataBuffers[tag]
    buf["x"].append(newX)
    buf["y"].append(newY)

    # Only keep data within the last windowSeconds
    cutoffTime = newX - windowSeconds
    while buf["x"] and buf["x"][0] < cutoffTime:
        buf["x"].pop(0)
        buf["y"].pop(0)
...
def updateRelay(textTag, buttonTag, newData):
...
# Update the software mode data
def updateSoftwareMode(textTag, currentMode):
    # Creating a map for the names
    MODE_NAMES = ["INIT", "TEST", "WAIT", "SEQUENCE", "SAFE", "SHUTDOWN"]
    # Creating a map for the colors
    SoftwareModesMap = {
        "INIT": (240, 255, 255, 255),
        "TEST": (240, 255, 255, 255),
        "WAIT": (240, 255, 255, 255),
        "SEQUENCE": (240, 255, 255, 255),
        "SAFE": (255, 0, 0, 255),
        "SHUTDOWN": (52, 215, 57, 178)
    }
    if currentMode is None or not isinstance(currentMode, (int, float)) or math.isnan(currentMode):
        print(f"[Warning] Software_mode invalid: {currentMode}")
        return
    modeIdx = int(currentMode)
    # Validates the tag and current mode is within range and updates said tag
    if dpg.does_item_exist(textTag) and 0 <= modeIdx <= 5:
        mode_name = MODE_NAMES[modeIdx]
        mode_color = SoftwareModesMap[mode_name]
        dpg.set_value(textTag, mode_name)
        dpg.configure_item(textTag, color=mode_color)
    else:
...
def updatePressureLine(status, lineTag, label):
...
def assumedPressure(tempData):
    ...
    n2oPressureFunc = interp1d(TEMPERATURE_K_DATA, PRESSURE_BAR_DATA, kind='linear', bounds_error=False, fill_value='extrapolate')

    n2oTempK = tempData + 273.15

    # Calculating pressure based on temp data
    pressure = n2oPressureFunc(n2oTempK)

    return pressure

# Function to calculate delta T
def temperatureChange(timeBuffer, tempBuffer):
...
def pressureDrop(timeBuffer, pressureBuffer):
    # Ensures we have more then 2 data point, if not we know the change is 0
    if len(timeBuffer) < 2:
        return 0.0
